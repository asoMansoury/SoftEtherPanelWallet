...
import  { RegisterUsersInDBForCisco, RegisterUsersInDBForOpenVpnTunnel } from "src/databse/user/registerusers";
    ...
    if (req.method === 'POST') {
      ...
      // Process the data or perform any necessary operations;
       if(usersBasketObj != null){
        var registerCustomer  =  await RegisterCustomers(usersBasketObj,apiUrls.types.OpenVpn);
        ...
        await CalculateWallet(registerCustomer.email,apiUrls.types.OpenVpn,usersBasketObj.price,usersBasketObj);
        //put the new user into the database
        var userRegistered = [];
        await Promise.all(newUsers.map(async (userNew) => {
        var insertedUser = await RegisterUsersInDBForOpenVpnTunnel(servers, 
                                                            userNew,
                                                            apiUrls.types.OpenVpn,
        ...
        userRegistered.map((userItem,userIndex)=>{
            CreateUserOnOpenVpn(selectedServer,userItem,userItem.expires);
        })
        ...
        userRegistered.map((userItem,userIndex)=>{
          ...
          servers.map((server,index)=>{
            if(server.isactive){
                ...
                userItem.ovpnurl = server.ovpnurl;
