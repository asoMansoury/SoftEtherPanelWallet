...
const Basket = () => {
  ...
  useEffect(async()=>{
    await axios.get(apiUrls.localUrl.getTariffsUrl+apiUrls.types.OpenVpn).then(data =>{
    ...
    if(status ==="authenticated"){
      ...
      await axios.get(apiUrls.agentUrl.isAgentUrl+profileSelector.email).then(response=>{
        if(response.data.name.isAgent==true){
          ...
          var url = apiUrls.agentUrl.getAgentInformation+response.data.name.agentcode+`&type=${apiUrls.types.OpenVpn}`;
  ...
  async function getAgentInformation(agentCode){
    setAgent(agentCode);
    var url = apiUrls.agentUrl.getAgentInformation+agentCode+`&type=${apiUrls.types.OpenVpn}`;
    axios.get(url).then(data =>{
        ...
        <Grid item xs={12}>

          {
            (agentInformation&& agent!='nobody')&&status=="authenticated"&&
              <FormLayoutTypeBasket typeVpn = {apiUrls.types.OpenVpn} agentData={agentInformation} tariffs={tariffs} agent={agent} />
          }
                    {
