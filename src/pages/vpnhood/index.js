...
const Cisco = () => {
  ...
  useEffect(async()=>{
    ...
    await axios.get(apiUrls.localUrl.getTariffsUrl+apiUrls.types.VpnHood).then(data =>{
  ...
  async function getAgentInformation(agentCode){
    setAgent(agentCode);
    var url = apiUrls.agentUrl.getAgentInformation+agentCode+`&type=${apiUrls.types.VpnHood}`;
        ...
        <Grid item xs={12}>
          {
            ...
            <FormLayoutTypeBasket typeVpn = {apiUrls.types.VpnHood} agentData={agentInformation} tariffs={tariffs} agent={agent} />
