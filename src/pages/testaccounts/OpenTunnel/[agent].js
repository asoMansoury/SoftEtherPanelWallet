...
const index = () => {
  ...
  useEffect(async () => {
    ...
    if (profileSelector.isLoggedIn == true) {
      var isUserValid = (await axios.get(apiUrls.testAccountsUrls.isvalid + profileSelector.email + "&type=" + apiUrls.types.OpenVpn)).data;
  ...
  async function LoadServers(){
    setIsEnabledConfirm(true);
    var servers = await axios.get(apiUrls.server.getServersForTestApi + apiUrls.types.OpenVpn);
    var tmp = [];
  ...
  async function GetTestAccount(e) {
    ...
    if (isUserValid.name.isValid == true) {
      ...
      var generateAccount = (await axios.get(apiUrls.testAccountsUrls.gettestaccount + email + "&type=" + apiUrls.types.OpenVpn+"&servercode="+selectedServer+"&agentcode="+agentCode));
      setError({
