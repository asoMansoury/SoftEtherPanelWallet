...
const IncAgentWalletComponent = (props) => {
    ...
    async function btnSubmitHandler(e) {
        ...
        const obj = {
            money:data.refundMoney,
            agentcode:data.agentcode,
            email:data.email
        }
        const result = await axios.post(apiUrls.agentUrl.increateAgentWallet,obj);
        if(result.data.result.isValid==true){
            setError({
                isValid:false,
                errosMsg:"عملیات افزایش اعتبار با موفقیت انجام گردید",
                severity:'success'
            });
    ...
    return <div>
        {
            ...
            <Grid container spacing={6}>
                <Grid item xs={12}>
                    <Card>
                        <CardHeader title="افزایش اعتبار نماینده فروش" titleTypographyProps={{ variant: 'h6' }} />
                        <Divider sx={{ margin: 0 }} />
                        {
                            error.isValid == false &&
                        ...
                        <CardContent>
                            <Grid container spacing={6}>
                                ...
                                <Divider></Divider>
                                <Grid item xs={12} sm={6}>
                                    <TextField fullWidth label='افزایش مبلغ' placeholder='افزایش مبلغ' value={data.refundMoney} onChange={refundMoneyHandler} />
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    {
...
export default IncAgentWalletComponent
