...
export async function sendEmailVpnHoodClient(to, users, accessTokenResult, subject, currentDomain, customer) {
  console.log("Send Email Flag : ", process.env.SEND_EMAIL)
  if (process.env.SEND_EMAIL == 'false') {
    return;
  }
  ...
  try {
    const tableRows = users.map((user, index) =>
      ...
      <tr style="background-color: ${index % 2 === 0 ? '#f9f9f9' : '#ffffff'};">
        ...
        <td style="padding: 10px; border: 1px solid #ddd; color: #555555;">${user.password}</td>
        <td style="padding: 10px; border: 1px solid #ddd; color: #555555; font-weight: bold; color: #5b2121;">${user.expires}</td>
        <td style="padding: 10px; border: 1px solid #ddd; color: #555555;">${accessTokenResult.accessToken}</td>
      </tr>
    `).join('');
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <thead style="background-color: #f1f1f1;">
              <tr>
                ...
                <th style="padding: 10px; border: 1px solid #ddd;">کلمه عبور</th>
                <th style="padding: 10px; border: 1px solid #ddd;">تاریخ انقضا</th>
                <th style="padding: 10px; border: 1px solid #ddd;">کد کپی گردد.</th>
              </tr>
            </thead>
            <tbody>
              ${tableRows}
...
export async function sendEmailVpnHoodClientTest(to, users, accessTokenResult, subject, agentInformation) {
  console.log("Send Email Flag : ", process.env.SEND_EMAIL)
  if (process.env.SEND_EMAIL == 'false') {
    return;
  }
  ...
  try {
    const tableRows = users.map((user, index) =>
      ...
      <tr style="background-color: ${index % 2 === 0 ? '#f9f9f9' : '#ffffff'};">
        ...
        <td style="padding: 10px; border: 1px solid #ddd; color: #555555;">${user.password}</td>
        <td style="padding: 10px; border: 1px solid #ddd; color: #555555; font-weight: bold; color: #5b2121;">${user.expires}</td>
        <td style="padding: 10px; border: 1px solid #ddd; color: #555555;"><span>${accessTokenResult}</span></td>
      </tr>
    `).join('');
    ...
    const table = `
        <div>
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <thead style="background-color: #f1f1f1;">
              <tr>
                ...
                <th style="padding: 10px; border: 1px solid #ddd;">کلمه عبور</th>
                <th style="padding: 10px; border: 1px solid #ddd;">تاریخ انقضا</th>
                <th style="padding: 10px; border: 1px solid #ddd;">کد دسترسی</th>
              </tr>
            </thead>
            <tbody>
              ${tableRows}
          ...
          <div dir="rtl" style=" margin-top: 8px; padding: 15px; border: 1px solid #ccc; border-radius: 5px; background-color: #f5f5f5;">
            <p style="margin-bottom: 8px; font-size: 16px; font-weight: bold; color: red;">کد دسترسی را کپی کنید و وارد نرم افزار شوید و داخل باکس سرورها رویه گزینه افزودن کلیک کنید و کد را وارد نمایید.</p>
            ${telegramLink}
          </div>
          <divdir="rtl" style=" margin-top: 8px; padding: 15px; border: 1px solid #ccc; border-radius: 5px; background-color: #f5f5f5;">
          ${botTelegram}
